...
numPerms <- length(actualLabels)

empiricalPValue <- (numPermsGreater + 1) / (numPerms + 1)
```
...
For the permutation analysis, you would repeat classification using the permuted group labels. After repeating this process (random shuffling + classification) many times, you would compare the accuracy you saw with the *actual* labels against the accuracy you saw with the *randomly* permuted labels.

To illustrate how the case/control values are permuted, we will represent which individuals are cases and controls using a vector called ```caseControlStatus```. A value of ```1``` indicates that the sample at that index is a case. A value of ```0``` indicates that a sample is a control.
...
#### As stated below, there are 252 unique ways to permute this vector. Thus, all of the permutations will be repeated approximately 1000 / 252 = ~4 times each.
...
#### In the end, duplicates will affect our p-value, since they will either increase our decrease our numerator, depending on where our initial observed accuracy lies. 

## A problem

As mentioned above, one limitation of current permutation approaches is that they do not ensure that all of the generated permutations are unique. "This scenario requires careful treatment from a mathematical point of view, because of the possibility that the random permutations may include repetitions of the same permutations and indeed of the original data." Repeats in a permutation analysis may skew the p-values that are generated from this process (making them either too strict or too conservative). Therefore, it is important to take measures to ensure that each permutation is unique. Refer to [this article](http://www.degruyter.com/view/j/sagmb.2010.9.1/sagmb.2010.9.1.1585/sagmb.2010.9.1.1585.xml) for a more in-depth explanation of this concept.

In some cases, repeats are unavoidable. As we demonstrated, if you have 10 samples (and 5 cases), it is only possible to generate 252 unique permutations. In this scenario, a permutation test might not be the best technique to use.

Now consider the scenario where you have a much larger data set--let's say 1000 samples and 500 cases. There would be an incredibly large number of ways to permute the data (see below). In this case, it would be very unlikely that many of these permutations would be duplicates, so we wouldn't need to worry too much about excluding duplicates.
...
Let's say you had 50 samples and only 5 controls and you wanted to generate 100,000 permutations. On average, about 2324 of those permutations would be duplicates, thus potentially biasing your results.
...
getUniquePerms <- function(groupAssignments, numPerms) {
  ...
  upperBound <- min(numPerms, choose(length(groupAssignments), length(which(caseControlStatus == 1))) - 1)
  # I treat the assignments vector as a binary string and just convert it to decimal
  permValue <- unbinary(paste(groupAssignments, collapse = ""))
  uniquePermVals <- c(permValue, uniquePermVals)

  uniquePermutations <- NULL
  numPermsCheck <- 0

  while (numPermsCheck != upperBound) {
    # Get new permutation
    permutedGroup <- sample(groupAssignments, replace = FALSE)
    # Calculate permutation value for a new group
    permValue <- unbinary(paste(permutedGroup, collapse = ""))
    # If it is a unique permutation
    if (!(permValue %in% uniquePermVals)) {
      uniquePermVals <- c(permValue, uniquePermVals)
      uniquePermutations <- rbind(permutedGroup, uniquePermutations)
      numPermsCheck <- numPermsCheck + 1
    }
  }
